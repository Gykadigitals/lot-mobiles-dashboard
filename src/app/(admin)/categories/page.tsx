...
export default function CategoriesPage() {
  ...
  const handleAddCategory = () => {
    ...
    setData([{ id: newId, name: 'New Category', count: 100, startprice: 1999, bgImage: '', url: '/' }, ...data]);
    // Reset to first page so they see the newly added category
    setCurrentPage(1);
  };
  ...
  const handleImageUpload = (e: React.ChangeEvent<HTMLInputElement>, index: number) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        const newData = [...data];
        newData[index] = { ...newData[index], bgImage: reader.result as string };
        setData(newData);
      };
  ...
  if (loading) return <div className="p-6">Loading categories data...</div>;
  if (error) return <div className="p-6 text-red-500">{error}</div>;

  return (
    <div className="p-6 w-full mx-auto text-gray-900 dark:text-gray-100">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <h1 className="font-medium text-brand-500 text-theme-lg dark:text-white/90 sm:text-2xl">Categories Management</h1>
      ...
      <div className="bg-white dark:bg-white/[0.03] rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 p-6 space-y-6">
        {data.length === 0 && <p className="text-gray-500">No categories found.</p>}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {currentData.map((cat, i) => {
            ...
            <div key={actualIndex} className="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden bg-gray-50 dark:bg-white/[0.02]">
              {/* Image Preview */}
              <div className="h-40 bg-gray-200 dark:bg-gray-700 relative flex items-center justify-center group p-4">
                {cat.bgImage ? (
                  <img src={cat.bgImage} alt={cat.name} className="max-w-full max-h-full object-contain" />
                ) : (
              ...
              <div className="p-4 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const newData = [...data];
                        newData[actualIndex] = { ...cat, id: e.target.value };
                        setData(newData);
                      }}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const newData = [...data];
                        newData[actualIndex] = { ...cat, name: e.target.value };
                        setData(newData);
                      }}
                      className="w-full text-sm bg-white dark:bg-white/[0.03] border border-gray-300 dark:border-white/[0.08] rounded px-2 py-1.5 focus:outline-none focus:border-brand-500"
                    />
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <input
                      type="number"
                      value={cat.startprice}
                      onChange={(e) => {
                        const newData = [...data];
                        newData[actualIndex] = { ...cat, startprice: Number(e.target.value) };
                        setData(newData);
                      }}
                      className="w-full text-sm bg-white dark:bg-white/[0.03] border border-gray-300 dark:border-white/[0.08] rounded px-2 py-1.5 focus:outline-none focus:border-brand-500"
                    />
                  ...
                  <div>
                    <label className="text-xs text-gray-500 block mb-1">Item Count</label>
                    <input
                      type="number"
                      value={cat.count}
                      onChange={(e) => {
                        const newData = [...data];
                        newData[actualIndex] = { ...cat, count: Number(e.target.value) };
                        setData(newData);
                      }}
                      className="w-full text-sm bg-white dark:bg-white/[0.03] border border-gray-300 dark:border-white/[0.08] rounded px-2 py-1.5 focus:outline-none focus:border-brand-500"
                    />
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const newData = [...data];
                      newData[actualIndex] = { ...cat, url: e.target.value };
                      setData(newData);
                    }}
                    className="w-full text-sm bg-white dark:bg-white/[0.03] border border-gray-300 dark:border-white/[0.08] rounded px-2 py-1.5 focus:outline-none focus:border-brand-500"
                  />
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-800">
            <span className="text-sm text-gray-500">
              Showing {data.length === 0 ? 0 : startIndex + 1} to {Math.min(startIndex + itemsPerPage, data.length)} of {data.length} categories
            </span>
