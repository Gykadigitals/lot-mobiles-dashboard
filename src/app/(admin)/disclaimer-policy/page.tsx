...
export default function DisclaimerPolicyPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/disclaimer-policy', {
        method: 'POST',
      ...
      alert('Disclaimer Policy saved successfully!');
    } catch (e: any) {
      alert(e.message);
    }
    setSaving(false);
  ...
  if (loading) return <div className="p-6">Loading disclaimer policy data...</div>;
    ...
    <div className="p-6 w-full mx-auto text-gray-900 dark:text-gray-100">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <h1 className="font-medium text-brand-500 text-theme-lg dark:text-white/90 sm:text-2xl flex items-center gap-2">
          <FileText className="text-brand-500" /> Disclaimer Policy Management
        </h1>
