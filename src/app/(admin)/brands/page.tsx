...
export default function BrandsPage() {
  ...
  const handleAddBrand = () => {
    ...
    setData([{ id: newId, name: 'New Brand', image: '' }, ...data]);
    // Reset to first page so they see the newly added brand
    setCurrentPage(1);
  };
  ...
  if (loading) return <div className="p-6">Loading brands data...</div>;
  if (error) return <div className="p-6 text-red-500">{error}</div>;

  return (
    <div className="p-6 w-full mx-auto text-gray-900 dark:text-gray-100">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <h1 className="font-medium text-brand-500 text-theme-lg dark:text-white/90 sm:text-2xl">Brands Logo Management</h1>
      ...
      <div className="bg-white dark:bg-white/[0.03] rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 p-6 space-y-6">
        {data.length === 0 && <p className="text-gray-500">No brands found.</p>}
        
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {currentData.map((brand, i) => {
            ...
            <div key={actualIndex} className="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden bg-gray-50 dark:bg-white/[0.02]">
              {/* Image Preview */}
              <div className="h-32 bg-gray-200 dark:bg-gray-700 relative flex items-center justify-center group p-4">
                {brand.image ? (
              ...
              <div className="p-4 space-y-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const newData = [...data];
                      newData[actualIndex] = { ...brand, id: e.target.value };
                      setData(newData);
                    }}
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const newData = [...data];
                      newData[actualIndex] = { ...brand, name: e.target.value };
                      setData(newData);
                    }}
                    className="w-full text-sm bg-white dark:bg-white/[0.03] border border-gray-300 dark:border-white/[0.08] rounded px-2 py-1.5 focus:outline-none focus:border-brand-500"
                    placeholder="SAMSUNG"
                ...
                <div>
                  <label className="text-xs text-gray-500 block mb-1">Logo URL (or upload above)</label>
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={(e) => {
                        const newData = [...data];
                        newData[actualIndex] = { ...brand, image: e.target.value };
                        setData(newData);
                      }}
                      className="w-full text-sm bg-white dark:bg-white/[0.03] border border-gray-300 dark:border-white/[0.08] rounded px-2 py-1.5 focus:outline-none focus:border-brand-500"
                      placeholder="/images/brands/samsung.png"
                    />
                  </div>
                </div>
              </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-gray-800">
            <span className="text-sm text-gray-500">
              Showing {data.length === 0 ? 0 : startIndex + 1} to {Math.min(startIndex + itemsPerPage, data.length)} of {data.length} brands
            </span>
