...
const FRONTEND_FILE_PATH = '/Users/apple/Gyka Digitals /Developer Files /LOT Mobiles /lot-mobiles/src/data/replacement-policy.ts';
...
export async function GET() {
  try {
    ...
    // Extract the object part: export const replacementPolicyData: ReplacementPolicyData = { ... };
    const match = content.match(/export const replacementPolicyData: ReplacementPolicyData = ([\s\S]*);/);
    if (!match || !match[1]) {
...
export interface ReplacementPolicyData {
  pageTitle: string;
  pageSubtitle: string;
  lastUpdated: string;
  seoTitle: string;
...
export const replacementPolicyData: ReplacementPolicyData = ${JSON.stringify(data, null, 2)};
`;
