...
const FRONTEND_FILE_PATH = '/Users/apple/Gyka Digitals /Developer Files /LOT Mobiles /lot-mobiles/src/data/cancellation-policy.ts';
...
export async function GET() {
  try {
    ...
    // Extract the object part: export const cancellationPolicyData: CancellationPolicyData = { ... };
    const match = content.match(/export const cancellationPolicyData: CancellationPolicyData = ([\s\S]*);/);
    if (!match || !match[1]) {
...
export const cancellationPolicyData: CancellationPolicyData = ${JSON.stringify(data, null, 2)};
`;
