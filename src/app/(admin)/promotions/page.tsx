...
export default function PromotionsPage() {
  ...
  const handleAddBanner = () => {
    const maxId = data.reduce((max, b) => Math.max(max, b.id || 0), 0);
    // LIFO: Add to the beginning of the array
    setData([{ id: maxId + 1, img: '', url: '#' }, ...data]);
  ...
  if (loading) return <div className="p-6">Loading promotions data...</div>;
  if (error) return <div className="p-6 text-red-500">{error}</div>;

  return (
    <div className="p-6 w-full mx-auto text-gray-900 dark:text-gray-100">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <h1 className="font-medium text-brand-500 text-theme-lg dark:text-white/90 sm:text-2xl">Promotions Management</h1>
        <div className="flex items-center gap-3">
          {/* Add New Banner button moved here */}
          <button
      ...
      <div className="bg-white dark:bg-white/[0.03] rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 p-6 space-y-6">
        {data.length === 0 && <p className="text-gray-500">No promo banners found.</p>}
          ...
          {currentData.map((banner, i) => {
            ...
            <div key={actualIndex} className="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden bg-gray-50 dark:bg-white/[0.02]">
              ...
              <div className="h-40 bg-gray-200 dark:bg-gray-700 relative flex items-center justify-center group">
                {banner.img ? (
                  <img src={banner.img} alt="Promo preview" className="w-full h-full object-cover" />
                ) : (
              ...
              <div className="p-4 space-y-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      const newData = [...data];
                      newData[actualIndex] = { ...banner, id: Number(e.target.value) };
                      setData(newData);
                    }}
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      onChange={(e) => {
                        const newData = [...data];
                        newData[actualIndex] = { ...banner, img: e.target.value };
                        setData(newData);
                      }}
                      className="w-full text-sm bg-white dark:bg-white/[0.03] border border-gray-300 dark:border-white/[0.08] rounded px-2 py-1.5 focus:outline-none focus:border-brand-500"
