...
const FRONTEND_FILE_PATH = '/Users/apple/Gyka Digitals /Developer Files /LOT Mobiles /lot-mobiles/src/data/disclaimer-policy.ts';
...
export async function GET() {
  try {
    ...
    // Extract the object part: export const disclaimerPolicyData: DisclaimerPolicyData = { ... };
    const match = content.match(/export const disclaimerPolicyData: DisclaimerPolicyData = ([\s\S]*);/);
    if (!match || !match[1]) {
...
export interface DisclaimerPolicyData {
  pageTitle: string;
  pageSubtitle: string;
  lastUpdated: string;
  seoTitle: string;
...
export const disclaimerPolicyData: DisclaimerPolicyData = ${JSON.stringify(data, null, 2)};
`;
