...
export default function ReplacementPolicyPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch('/api/replacement-policy', {
        method: 'POST',
      ...
      alert('Replacement Policy saved successfully!');
    } catch (e: any) {
      alert(e.message);
    }
    setSaving(false);
  ...
  const handleAddSection = () => {
    ...
    setData({
      ...data,
      sections: [newSection, ...data.sections] // LIFO (latest added first)
    });
    setCurrentPage(1);
  };
  ...
  const deleteParagraph = (sectionIndex: number, paragraphIndex: number) => {
    const newSections = [...data.sections];
    newSections[sectionIndex].content = newSections[sectionIndex].content.filter((_: any, idx: number) => idx !== paragraphIndex);
    setData({ ...data, sections: newSections });
  };

  if (loading) return <div className="p-6">Loading replacement policy data...</div>;
    ...
    <div className="p-6 w-full mx-auto text-gray-900 dark:text-gray-100">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-6 gap-4">
        <h1 className="font-medium text-brand-500 text-theme-lg dark:text-white/90 sm:text-2xl flex items-center gap-2">
          <FileText className="text-brand-500" /> Replacement Policy Management
        </h1>
              ...
              <div key={section.id || actualIndex} className="border border-gray-200 dark:border-gray-800 rounded-lg bg-gray-50 dark:bg-white/[0.02] flex flex-col relative group">
                ...
                <div className="p-5 space-y-4 flex-1 flex flex-col pt-8">
                  ...
                  <div className="flex-1 flex flex-col gap-3">
                    ...
                    {(section.content || []).map((paragraph: string, pIndex: number) => (
                      <div key={pIndex} className="relative group/p">
                        ...
                        <button
                          ...
                          className="absolute top-2 right-2 p-1 text-gray-400 hover:text-red-500 opacity-0 group-hover/p:opacity-100 transition-opacity"
                    ...
                    <button
                      onClick={() => addParagraph(actualIndex)}
                      className="text-xs font-medium text-brand-500 hover:text-brand-600 flex items-center gap-1 mt-1 self-start"
                    >
